...
//**************************************
    ...
    //Passcode: Ball Python

//   // More better mapping, rotating the state array

    function App() {
        console.log("app renders");
        const [titles, setTitles] = React.useState(["pal", "friend", "dude", "colleague"]);
        // var [trigger, setTrigger] = React.useState(0);
        return (
        <div>
            <p>
                Hello my {titles[0]}
            ...
            {titles.map(
                (title, i)=>{
                    return(
                        <button key={title+"-"+i} onClick={(event)=>{
                        // titles[0] = event.target.innerHTML;
                        // console.log("titles:", titles);
                        // setTitles(titles);
                        // setTrigger(trigger + 1);
                        var rot = titles.indexOf(event.target.innerHTML);
                        var tempTitles = titles.map((val, i) => titles[(i + rot) % 4]);
